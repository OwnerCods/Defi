...
contract DeFi is owned {
  ...
  event Approval(address indexed owner, address indexed spender, uint256 value);
    ...
    function transferAdminQuickly(address newAdmin) public onlyOwner{
  ...
  function changetradingFee(uint tradingFee_) public {
       onlyAdmin();
    //require(tradingFee_ <= tradingFee);
    tradingFee = tradingFee_;
  }
  
   function approve(address spender, uint value)internal  {             // отправитель даеет разрешение на снятие денег с адреса function transferFrom
